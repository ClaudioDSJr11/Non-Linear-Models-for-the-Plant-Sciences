...
<center>![](Rust.jpg){width="700"}</center>
...
-   $r$
    ...
    -   Multiply $x_i$, but is on an **exponential**, each unit change in $\mathbf{x}$ is multiplicative in the growth (differential equation[^02-nonlinear-models-1]).
...
[^02-nonlinear-models-1]: $\frac{dy}{dt} = \mathbf{ry}(1-\frac{\mathbf{y}}{\mathbf{K}})$
...
This way we can get Wald confidence intervals[^02-nonlinear-models-2].

[^02-nonlinear-models-2]: Wald CI measure local uncertainty (around the parameter estimate) and not at the whole parameter space (all values it could assume - global), therefore, they might be more optimistic than global CI. Bootstrap or Bayesian approach would provide more closely a global uncertainty measure once they explore a broader region of the parameter space.
